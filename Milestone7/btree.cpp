...
BTreeTable::BTreeTable(Identifier table_name, ColumnNames column_names, ColumnAttributes column_attributes,
    ...
    cas = *get_column_attributes(cn);
    index = new BTreeFile(*this,table_name, primary_key, cn, cas, true);
}
...
ValueDict* row2 = validate(row);
index->insert_value(row2);
return Handle(*(index->tkey(row2)));
}
...
ValueDict* BTreeTable::project(Handle handle) {
    ...
    ValueDict *key = index->lookup_value(&vals);
	...
	for(auto const& cn : *key)
        (*result)[cn.first] = cn.second;
...
ValueDict* BTreeTable::project(Handle handle, const ColumnNames* column_names) {
    ...
    bool foundPKey;
    ValueDict* result = new ValueDict();
    ValueDict *vd = index->lookup_value(&vals);
    ...
    if(!cn.empty()){
        for(auto const& col : cn)
            (*result)[col] = vd->at(col);
    }
    return result;
}
...
ValueDict* BTreeTable::validate(const ValueDict* row) const {
    ...
    for(Identifier cn : column_names)
    {
        if(row->find(cn) == row->end())
...
bool BTreeTable::selected(Handle handle, const ValueDict* where) {
    ...
    for(auto const& cn : *where)
        if(row.at(cn.first) != where->at(cn.first))
            return false;
    return true;
...
bool test_helper(DbRelation &table, Handle handle, int a, std::string b) {
...
bool test_btable(){
    ...
    ColumnNames primaryKeyObj;
    primaryKeyObj.push_back("id");

    BTreeTable bTable("_test_btable", column_names, column_attributes, primaryKeyObj);
    bTable.create_if_not_exists();
    ValueDict *row;
    ValueDicts expected;
    Handles bHandles;
    std::string bVal = "testing testing";
    for(int i = 0; i < 100; ++i){
        row = new ValueDict;
        (*row)["id"] = Value(i);
        (*row)["a"] = Value(12);
        (*row)["b"] = Value("Much longer piece of text hereMuch longer piece of text hereMuch longer piece of text hereMuch longer piece of text hereMuch longer piece of text hereMuch longer piece of text hereMuch longer piece of text hereMuch longer piece of text hereMuch longer piece of text hereMuch longer piece of text here");
    ...
    for(auto const& handle : *bTable.select()){
        ...
        for(auto expect : expected) {
            if (test_helper(bTable, handle, (*expect)["a"].n, (*expect)["b"].s)) {
                bTest = true;
                break;
            }
        }
    ...
    for(auto const& handle : *bHandlesFound){
        if (!test_helper(bTable, handle, (*row)["a"].n, (*row)["b"].s)) {
            return false;
        }
    }
    for(int i = 0; i < 25; ++i){
